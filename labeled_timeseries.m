...
% accompnaying behavioral labels.


% Note that the output timeseries here may be
% shorter in length than the input timeseries; this is because any epochs
% whose start or endpoint is past the end of the timeseries will be
% dropped; if only the endpoint was past the end, then the part from the
% start point to the end of the original timeseries will be dropped.

% bhv_name_codes : Note that some values may be skipped if all instances
%                  of that behavior were too short to support the specified
%                  bin width and step size.

% aux.bhv_list      : list of retained behaviors; these are in alphabetical
%                     and their associated number codes are also in
%                     ascending order (though note that some bhvs/number
%                     codes can be skipped, namely if the epoch is too
%                     short to support the specified bin width and step
%                     size).
...
for i_sess = 1:n_sessions
    ...
    % sessions_raw because upon attempting to bin, it may come about that all instances
    % of a behavior were to short to support the desired bin width and
    % step. In that case, that behavior (and its corresponding timepoints)
    % will be skipped. Thus, after binning, we see what behaviors remain,
    % and save those as the behaviors for the current session.
