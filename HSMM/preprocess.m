%% Notes:
% This is a script based on hmm_preprocess_pipeline_5. It is used to
% generate the preprocessed data of male/female interaction and whole
% session data 
clearvars
close all
clc
...
%% Run setup for CNMF-E; add preprocessing/stats functions to path

if true
    addpath(genpath('/home/jmc/Dropbox (NYU Langone Health)/bhv-mfp/preprocessing'))
    rmpath('/home/jmc/Dropbox (NYU Langone Health)/bhv-mfp/preprocessing/archive')
end


%% Load data and preprocess

%   Animal ID, Session ID, Bin Size
subj_sessions ...
    = {1, '190912', 8; ...  % 1
       6, '200311', 8; ...  % 2 
       6, '200312', 8; ...  % 3 
...
params.portion = 'interaction';%'wholesession' or 'interaction'

for i_subj_sess = 1:n_subj_sess
    % Specify data location and subject to be analyzed. 
    params.path_to_data = '/home/jmc/Dropbox (NYU Langone Health)/bhv-mfp/mfp-data';
    
    % Set whether or not to z-score and/or smooth raw data after extraction.
    params.zscore_raw_extracted = false;
...
%         end
        ...
        for i_subsection = 1:params.n_subsections
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            run_id = 'B';
            % A means resampling all regions together with Intro and Rmv
            % epochs.
            % B means resampling regions separately with Intro and Rmv
            % epochs. Whole session versions stop here.
            % C means resampling regions separately without Intro and Rmv
            % epochs.
            % D mean resampling regions separately without Intro and Rmv
            % epoch but from a new preprocessing run where we will
            % preprocess and subdivide all sessions.
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if params.save_emp
                ...
                curr_dir = pwd;
                cd('/home/jmc/Dropbox (NYU Langone Health)/bhv-mfp/preprocessing/results/preprocessed-data/poisson-hmm/emp/')
                if params.resampling.resample
                    cd('resampled')
                else
                    cd('not-resampled')
            ...
            % Switch to save directory then back.
            curr_dir = pwd;
            cd('/home/jmc/Dropbox (NYU Langone Health)/bhv-mfp/preprocessing/results/preprocessed-data/poisson-hmm/sim/interaction/no-subsection/')
