%% Notes:
% This is a script visualize the partern PSID predict X, and
...
for ii = 1:length(select_session)
    ...
    male_idx = FrameS(idx_IntroM(end)):FrameE(idx_RmvM(end));
    ...
    for lag = 3
        ...
        for i = 1:length(select_session)
            for j = 1:length(select_session)
                ...
                tS_idx_attack = FrameS(idx_attack) - FrameS(idx_IntroM(end));
                tE_idx_attack = FrameE(idx_attack) - FrameS(idx_IntroM(end));
                idx_Minv = find(strcmp(behaviors,{'Investigate'}));
                idx_Minv = idx_Minv( idx_Minv > idx_IntroM(end) & idx_Minv < idx_RmvM(end));
                ...
                max_lines = min(length(tS_idx_attack),length(tS_idx_attack));
                ...
                for ii = 1:max_lines
                    [~,Sz,Vz] = svd(Sys_male.Cz);
                    temp = xTestPred(:,tS_idx_attack(select_trajectory(ii))-avewndw*lag:tS_idx_attack(select_trajectory(ii))+avewndw*lag-1);
                    bin_pred = [];
                    ...
                    duration(1,ii) = -tS_idx_attack(select_trajectory(ii))+tE_idx_attack(select_trajectory(ii));
